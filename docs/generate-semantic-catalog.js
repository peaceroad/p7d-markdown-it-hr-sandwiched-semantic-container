...
const EN_NOTES = {
  ...
  "appendix-titlepage": "No default `role` is emitted. For ebook title pages, prefer hr-sandwiched `h1` inference such as `Appendix A. Reference Data`, or parsed frontmatter `sc.titlepage: true` / `sc: { titlepage: true }`. The Policy section explains explicit-label visibility, hidden-label behavior, and accessible named groups shared by all titlepage semantics.",
  "chapter-titlepage": "No default `role` is emitted. For ebook title pages, prefer hr-sandwiched `h1` inference such as `Chapter 1. Title`, or parsed frontmatter `sc.titlepage: true` / `sc: { titlepage: true }`. The Policy section explains explicit-label visibility, hidden-label behavior, and accessible named groups shared by all titlepage semantics.",
  ...
  "related-book": "This is the canonical semantic for related books, magazines, and other publications. `related-publication` and natural publication forms are input aliases; output and canonical-name configuration use `sc-related-book` / `related-book`.",
  ...
  "part-titlepage": "No default `role` is emitted. For ebook title pages, prefer hr-sandwiched `h1` inference such as `Part 1. Title`, or parsed frontmatter `sc.titlepage: true` / `sc: { titlepage: true }`. The Policy section explains explicit-label visibility, hidden-label behavior, and accessible named groups shared by all titlepage semantics.",
...
const JA_NOTES = {
  ...
  "appendix": "`付属書`・`附属書`は規格・条約・技術文書で独立した後付け構造を表すため認識し、資料を明示する`付属資料`・`附属資料`もここに置きます。書籍で`付録A`と同様に使われる`付属A`・`附属A`などの短い番号・文字付き見出しも、通常の`appendix` labelとして認識します。`付属`・`附属`単独は、付属品や所属関係まで表せて文書構造を保証しないため認識しません。hrサンドイッチ`h1`の`付属A 参考データ`・`附属A 参考データ`は、より限定的な`appendix-titlepage`推定になります。",
  "appendix-titlepage": "DPUB-ARIAに近いroleがないため、既定の`role`属性は出力しません。電子書籍の扉では、`付録A 参考データ`/`付属A 参考データ`/`附属A 参考データ`のようなhrで挟まれた`h1`、または解析済みfrontmatterの`sc.titlepage: true` / `sc: { titlepage: true }`を優先します。明示ラベルの表示、非表示時の扱い、アクセシブルな名前付きグループについては、すべてのtitlepage semanticに共通する「方針」の説明を参照してください。",
  "chapter-titlepage": "DPUB-ARIAに近いroleがないため、既定の`role`属性は出力しません。電子書籍の章扉では、`第1章 はじめに`、`第II章 ローマ数字`のようなhrで挟まれた`h1`、または解析済みfrontmatterの`sc.titlepage: true` / `sc: { titlepage: true }`を優先します。明示ラベルの表示、非表示時の扱い、アクセシブルな名前付きグループについては、すべてのtitlepage semanticに共通する「方針」の説明を参照してください。",
  ...
  "book": "書籍・雑誌・刊行物の情報ブロックを表すcanonical semanticで、`sc-book`を出力します。`publication`、`magazine`と、それぞれの情報形式も入力ラベルとして認識します。関連・推薦刊行物は`related-book`、現在の刊行物自身の正式な奥付は`colophon`を使います。複数形`publications`は著者の業績一覧や刊行物一覧にもなるため、既定aliasにはしません。",
  ...
  "assessment": "アセスメント、試験、確認テストなど、測定・診断・確認を主目的とする部分に使います。`テスト`単独は技術文書で広すぎるため認識しません。`採点`単独は点数・評定を付ける行為なので、`assessment`として認識しません。canonicalは現行EPUB語彙の単数形`assessment`に合わせ、複数形`assessments`は英語aliasとして認識します。",
  "evaluation": "品質・価値・成果・性能などを判断する部分です。英語ではproduct/quality/performance evaluationの自然な単数形・複数形を認識します。日本語の`評価`、`評価結果`、`総合評価`は単独見出しとして自然です。出力はroleなしの`section`で、assessmentやgradingに固有のアクセシビリティ上の意味を付けないため、実用的なaliasとして認識します。より限定的な`製品評価`、`品質評価`、`性能評価`、`パフォーマンス評価`も認識します。一方、測定・診断寄りの`リスク評価`、grading寄りの`成績評価`、行為としての`採点`は認識しません。`評価基準`は`rubric`です。",
  ...
  "pullquote": "canonicalは現行EPUB語彙と`doc-pullquote`の綴りに合わせた`pullquote`です。`pull quote`と`pull-quote`は入力aliasで、出力classとcanonical名を使う設定は`sc-pullquote` / `pullquote`です。DPUB-ARIAでは本文と重複する装飾用プルクオートを支援技術から隠す必要がありますが、プラグインには原文と複製を判定できないため、`aria-hidden`を無条件には付けません。重複表示する場合は後段の変換で指定してください。",
  ...
  "learning-objective": "canonicalは現行EPUB語彙の単数形`learning-objective`に合わせます。`learning-objectives`と、自然な空白区切りの単数形・複数形を英語aliasとして認識します。業務目標やプロジェクト目標にもなる`objectives`単独は認識しません。EPUB側に対応するDPUB-ARIA roleがないため、既定roleは出力しません。",
  ...
  "related-book": "関連する書籍・雑誌・刊行物を扱うcanonical semanticです。`related-publication`とpublicationを含む自然な表現は入力aliasで、出力classとcanonical名を使う設定は`sc-related-book` / `related-book`です。",
  ...
  "part-titlepage": "DPUB-ARIAに近いroleがないため、既定の`role`属性は出力しません。電子書籍の部扉では、`第1部 扉タイトル`のようなhrで挟まれた`h1`、または解析済みfrontmatterの`sc.titlepage: true` / `sc: { titlepage: true }`を優先します。明示ラベルの表示、非表示時の扱い、アクセシブルな名前付きグループについては、すべてのtitlepage semanticに共通する「方針」の説明を参照してください。",
  "planning": "`計画`、`計画案`、`プラン`のような予定・段取りに使います。承認や採否判断を求めることを明示した企画案・提案書・企画書は`proposal`、軽い提案は`suggestion`を使います。",
  "point": "英語の`point`には論旨・要点・主眼という意味があるため、`要点`はここで維持します。中心的な要点なのでrolelessの`section`とし、助言を示す`hint`/`tip`の`doc-tip` asideとは分けます。",
  ...
  "supplement": "本文を明確にしたり補ったりする補足情報・補遺に使います。正式な後付け構造で`doc-appendix`を持つ`appendix`/`付録`とは分けます。`補遺`は使用頻度が低くても文書用語として意味が限定されているため維持します。刊行後の追加・修正を指す`追補`はaddendum寄りなので認識せず、後から書き足す`追記`は`postscript`に分けます。",
